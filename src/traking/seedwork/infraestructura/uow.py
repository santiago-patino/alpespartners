...
from traking.seedwork.dominio.entidades import AgregacionRaiz
...
def registrar_unidad_de_trabajo(serialized_obj):
    from cliente.config.uow import UnidadTrabajoSQLAlchemy
    from flask import session
...
def flask_uow():
    from flask import session
    from traking.config.uow import UnidadTrabajoSQLAlchemy, UnidadTrabajoPulsar
    if session.get('uow'):
        return session['uow']
